...
- [x] 部署后复查线上页面源码：普通组合页应有 `noindex, follow`，机会页应允许索引（2026-05-16 抽查通过；7 个机会页均为 `index, follow`，普通组合页抽查均为 `noindex, follow`；sitemap 共 29 个 URL：首页、Terms、20 个主 Arena 页、6 个 arena/card 机会页和 1 个 high-arenas 机会页）
- [ ] Arena 15-20 主页面补独特攻略段落：选第一套牌、常见失败原因、缺卡替换思路、适合玩家类型
- [x] 做“缺卡替换建议”：当用户差 1-2 张卡时，给同 arena 可用、相近费用/定位的替代卡（2026-05-16：规则驱动推荐，优先用户已选择卡牌，按角色/费用/当前 arena 可用性/卡组池出现频率排序）
...
**P3：基本工具能力（加分项，可选）**
- [x] 排序/筛选功能（2026-05-04：按胜率/使用率/样本量/费用排序，支持样本量和费用区间筛选）
...
- [x] 缺卡替换建议（优先级高于卡组对比）：当用户差 1-2 张卡时，给同 arena 可用、相近费用/定位的替代卡（2026-05-16 完成）
...
**工具体验优化（P2）**
- [x] 可替换卡牌建议（已完成：缺卡 1-2 张时展示同 arena 可用的相近费用/定位替换卡牌）
